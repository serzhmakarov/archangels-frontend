...
const PaymentFormModal = ({ onCloseModal, isOpen }) => {
  ...
  const [amount, setAmount] = useState(
    currencies[defaultCurrency].defaultAmounts[0],
  );
  ...
  const onRequest = ({ amount, currency }) => {
    getCheckoutFondyUrl({ amount, currency }).then((res) => console.log(res));
  };
  ...
  return (
    <Modal show={isOpen} onHide={onCloseModal} className="payment-form-modal">
      ...
      <Container className="currency-form">
        <Form onSubmit={handleSubmit}>
          ...
          <Modal.Footer>
            <Button variant="danger" className="payment-btn" type="submit">
